...
export type WatchDiscoveryItem = {
  id: number
  title?: string
  name?: string
...
const fetchSource = async (source: WatchCategorySource): Promise<WatchDiscoveryItem[]> => {
  ...
  let items: Omit<WatchDiscoveryItem, 'media_type'>[]
  ...
  switch (source.kind) {
    ...
    case 'filtered-movies': mediaType = 'movie'; items = await fetchMoviesByFilters(source.filters); break
  }
...
const interleave = (groups: WatchDiscoveryItem[][]) => {
  ...
  for (let index = 0; index < maxLength; index += 1) {
    for (const group of groups) if (group[index]) items.push(group[index])
  }
  return items
}

export async function fetchWatchCategory(category: WatchCategory) {
