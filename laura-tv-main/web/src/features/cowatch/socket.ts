...
import { cowatchUrl } from './config'

export const cowatchSocket = io(cowatchUrl, {
  autoConnect: false,
...
export function emitCowatchWithAck<T>(event: string, payload: object): Promise<T> {
  ...
  return new Promise<T>((resolve, reject) => {
    ...
    cowatchSocket.timeout(COWATCH_ACK_TIMEOUT_MS).emit(event, payload, (timeoutError: Error | null, result: T) => {
      if (timeoutError) return finish(() => reject(new Error('Cowatch did not respond in time. Please try again.')))
